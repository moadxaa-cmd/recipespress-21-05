...
const LogEntryItem: React.FC<{ entry: LogEntry }> = ({ entry }) => {
    ...
    return (
        <div className="border border-slate-200 rounded-lg bg-white">
            ...
            {isOpen && (
                <div className="p-4 border-t border-slate-200 bg-slate-50 text-sm rounded-b-lg max-h-96 overflow-y-auto">
...
export const ApiLogViewer: React.FC<ApiLogViewerProps> = ({ apiToken, showToast }) => {
    ...
    const fetchLogs = useCallback(async () => {
        ...
        setError(null);
        if (!apiToken) {
            setError("API Token is not configured. Cannot fetch logs.");
            setIsLoading(false);
            return;
        }
        try {
            const fetchedLogs = await getLogs(apiToken);
            setLogs(fetchedLogs);
        } catch (err) {
    ...
    const handleClearLogs = async () => {
        if (window.confirm('Are you sure you want to delete all API logs? This action cannot be undone.')) {
            try {
                await clearLogs(apiToken);
                setLogs([]);
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold text-slate-900">Backend API Logs</h2>
                <p className="text-slate-500 mt-1">
                    Shows logs from your backend API endpoint. Requires a Redis database to be configured.
                </p>
            </div>

            <div className="flex justify-end gap-3">
                <button onClick={fetchLogs} disabled={isLoading || !apiToken} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50 transition-colors">
                    {isLoading ? 'Refreshing...' : 'Refresh'}
                </button>
                <button onClick={handleClearLogs} disabled={!apiToken || logs.length === 0} className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-lg hover:bg-red-700 disabled:bg-slate-400 transition-colors shadow-sm">
            ...
            <div className="space-y-3">
                {isLoading ? (
                    ...
                    <div className="text-center py-16 text-slate-500 border-2 border-dashed border-red-200 rounded-lg bg-red-50">
                        <div className="inline-block p-4 bg-red-100 rounded-full">{React.cloneElement(Icons.exclamationCircle, {className: "h-8 w-8 text-red-500"})}</div>
                        <h3 className="mt-4 text-lg font-semibold text-red-800">Error Fetching Logs</h3>
                    ...
                    <div className="text-center py-16 text-slate-500 border-2 border-dashed rounded-lg">
                        ...
                        <p className="mt-1 text-sm">Your backend has not logged any API calls yet.</p>
                    </div>
                ) : (
