...
export const Dashboard: React.FC<DashboardProps> = ({ sites, addItemsToQueue, updatePostInHistory, showToast, setView, geminiApiKey, articleAgentSettings, adminSettings, refreshPostsTrigger, setRefreshPostsTrigger }) => {
  ...
  const [viewState, setViewState] = useState<'selection' | 'create' | 'update'>('selection');
  ...
  const checkApiKey = () => {
    if (!geminiApiKey) {
      showToast({ message: 'Please set your Gemini API Key first.', type: 'error', persistent: true, action: { label: 'Go to Settings', onClick: () => setView('settings', 'gemini') } });
      return false;
    }
    return true;
  }
