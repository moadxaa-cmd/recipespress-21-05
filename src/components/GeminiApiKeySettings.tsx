...
export const GeminiApiKeySettings: React.FC<GeminiApiKeySettingsProps> = ({ geminiApiKey, setGeminiApiKey, geminiModel, setGeminiModel, showToast, isQueuePaused }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isQueuePaused) {
      showToast('API settings updated. You can now resume the generation queue.');
    }
  };
    ...
    <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div className="pt-4 border-t border-slate-200">
            <label htmlFor="gemini-model" className="block text-sm font-medium text-slate-700">Preferred AI Model</label>
            <div className="mt-1">
              <select
                id="gemini-model"
                value={model}
                onChange={e => setModel(e.target.value)}
                className="block w-full pl-3 pr-10 py-2 text-base border-slate-300 focus:outline-none focus:ring-teal-500 focus:border-teal-500 sm:text-sm rounded-lg"
              >
                <optgroup label="Free Tier (Default)">
                    <option value="gemini-3-flash-preview">Gemini 3 Flash Preview (Fast, Free)</option>
                    <option value="gemini-2.5-flash">Gemini 2.5 Flash (Fast, Free)</option>
                </optgroup>
                <optgroup label="Paid Tier (Requires Billing enabled in Google Cloud)">
                    <option value="gemini-2.5-pro">Gemini 2.5 Pro (Powerful, Paid)</option>
                    <option value="gemini-1.5-pro">Gemini 1.5 Pro (Legacy, Paid)</option>
                </optgroup>
              </select>
            </div>
            <p className="mt-2 text-sm text-slate-500">
              Note: Pro models require a paid Google Cloud account linked to your API key, otherwise generations will fail. "Gemini 3 Flash Preview" is recommended for most users.
            </p>
          </div>
