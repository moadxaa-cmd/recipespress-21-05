...
export const ArticleAgentSettings: React.FC<ArticleAgentSettingsProps> = ({ settings, setSettings, showToast }) => {
  ...
  const handleBulkUploadAffiliates = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    const processData = (data: any[]) => {
      const newLinks: AffiliateLink[] = [];
      data.forEach((row, rowIndex) => {
        ...
        if (Array.isArray(row)) {
           ...
           if (keys.length >= 2) {
             productName = row[keys[0]];
             url = row[keys[1]];
           }
        }
        
        // Skip header-like rows roughly
        if (rowIndex === 0 && String(url).toLowerCase().includes('url')) {
            return;
        }

        if (productName && url && String(url).trim().startsWith('http')) {
           newLinks.push({ productName: String(productName).trim(), url: String(url).trim() });
        }
      });
      ...
      if (newLinks.length > 0) {
        setFormState(prev => ({
          ...prev,
          affiliateLinks: [...(prev.affiliateLinks || []), ...newLinks]
        }));
        showToast(`Imported ${newLinks.length} affiliate links successfully.`, 'success');
      } else {
        showToast('No valid links found. Ensure 2 columns: Product Name and URL.', 'error');
      }
    };
        ...
        <div className="p-6 bg-white rounded-lg border border-slate-200">
            <div className="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Provide a list of products and their links. The AI will intelligently insert them where relevant.</p>
                </div>
                <div>
                    <label className="cursor-pointer inline-flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-teal-700 bg-teal-50 hover:bg-teal-100 rounded-md transition-colors">
                        {Icons.upload} Upload CSV/Excel
                        <input type="file" accept=".csv, .xlsx, .xls" className="hidden" onChange={handleBulkUploadAffiliates} />
                    </label>
                </div>
            </div>
