...
function safeJsonParse(jsonString: string, context: string): Record<string, unknown> {
    ...
    try {
        ...
        throw new Error(`AI returned malformed JSON for ${context}. Please try again. The raw response was logged to the console.`, { cause: e });
    }
}

async function extractTitleFromText(apiKey: string, geminiModel: string, text: string): Promise<string> {
...
export async function generateImage(apiKey: string, prompt: string, _inputImage?: { imageBytes: string, mimeType: string }): Promise<string> {
    const ai = new GoogleGenAI({ apiKey });
...
async function handleGeneration(
    ...
    let { geminiModel = 'gemini-3-flash-preview', primaryKeyword, generationType, source, settings, adminSettings, imageConfig, existingPosts, imageStrategy = 'regenerate', language, siteCategories, siteUrl } = params;
    ...
    if (generationType !== 'intro' && source.type === 'text' && !primaryKeyword) {
        primaryKeyword = await extractTitleFromText(apiKey, geminiModel, source.value);
        if (!primaryKeyword) {
    ...
    if (siteCategories && siteCategories.length > 0) {
        try {
            ...
            detectedCategoryIds = await detectCategory(apiKey, geminiModel, primaryKeyword, sourceText, siteCategories);
    ...
    if (generationType === 'full' || generationType === 'seo-article') {
        ...
        generatedPost = await generateRecipeWithIntro(apiKey, geminiModel, source, primaryKeyword, normalizedLanguage, categoryContext, siteContext);
    }
...
async function agentOrchestrator(
    ...
    const userAffiliateLinks = settings.affiliateLinks && settings.affiliateLinks.length > 0 
        ? settings.affiliateLinks.map(l => `- Product: "${l.productName}", URL: "${l.url}"`).join('\n') 
        : '';
    
    const languageInstruction = `You MUST write the entire article and all fields in ${language}.`;
    const externalLinksCount = Math.max(1, settings.externalLinks);
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('API key not valid')) {
                throw new Error("Your Gemini API key is not valid. Please check it in the settings.", { cause: error });
            }
            if (error.message.includes('429')) {
                throw new Error("API rate limit exceeded. Please wait a moment and try again.", { cause: error });
            }
             if (error.message.includes('response is missing required fields')) {
                 throw new Error("AI response validation failed. It might be incomplete. Please try again.", { cause: error });
             }
        }
        throw new Error("Failed to generate content. The AI model may be temporarily unavailable or there was a network issue.", { cause: error });
    }
}

export async function generateCategoryDetails(apiKey: string, name: string, geminiModel: string = "gemini-3-flash-preview"): Promise<{ slug: string, description: string }> {
    const ai = new GoogleGenAI({ apiKey });
    const prompt = `Generate an SEO-friendly slug and description for a recipe website category named "${name}".
Respond strictly in JSON format:
{
  "slug": "seo-friendly-slug-here",
  "description": "Short SEO description here (1-2 sentences)."
}`;
    ...
    try {
        ...
        return safeJsonParse(response.text, "Category Generation");
    } catch (e) {
        console.error("Failed to generate category details", e);
        return { slug: name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''), description: `Delicious ${name} recipes.` };
    }
}
export async function extractRecipeForEbook(apiKey: string, title: string, content: string, url: string, geminiModel: string = "gemini-3-flash-preview"): Promise<RecipeData> {
    const ai = new GoogleGenAI({ apiKey });
    
    // We expect the result to strictly follow the RecipeData interface
    const prompt = `You are a culinary expert. Extract the recipe details from the following content into a structured JSON format.
Content Title: ${title}
Content URL: ${url}
...
If any data is missing, make a reasonable guess or leave it as an empty string/array. 
...
${content}`;
    ...
    try {
        ...
        const parsed = safeJsonParse(response.text, "EBook Recipe Extraction");
        return {
            name: parsed.name || title || "Unknown Recipe",
            description: parsed.description || "",
            prep_time: parsed.prep_time || "N/A",
            cook_time: parsed.cook_time || "N/A",
            total_time: parsed.total_time || "N/A",
            yield: parsed.yield || "N/A",
            ingredients: Array.isArray(parsed.ingredients) ? parsed.ingredients : [],
            instructions: Array.isArray(parsed.instructions) ? parsed.instructions : [],
            difficulty: parsed.difficulty || "Medium",
            notes: parsed.notes || "",
            nutrition: parsed.nutrition || { calories: "" }
        };
    } catch (e) {
        console.error("Failed to extract recipe for eBook", e);
        throw e; // Rethrow to let the caller handle it
    }
}

export async function generateEbookCover(apiKey: string, prompt: string, title: string, author: string): Promise<string> {
    const ai = new GoogleGenAI({ apiKey });
    
    const imagePrompt = `A beautiful, minimalist, and appetizing book cover design for a recipe book titled "${title}" by "${author}". The design should be elegant, professional, without text (so we can overlay our own text). Mood: high quality culinary photography or illustration. Background should have some negative space. ${prompt}`;
    ...
    try {
        ...
        throw new Error("No image generated for cover");
    } catch (e) {
        console.error("Failed to generate ebook cover", e);
        throw e;
    }
}

export async function generateEbookIntro(apiKey: string, recipeNames: string[], geminiModel: string = "gemini-3-flash-preview"): Promise<string> {
    const ai = new GoogleGenAI({ apiKey });
    
    const prompt = `Write a warm, engaging, and appetizing introduction for a recipe ebook. 
The ebook contains the following recipes:
${recipeNames.map(name => '- ' + name).join('\\n')}

Keep it under 3 paragraphs. It should make the reader excited to start cooking.`;
    ...
    try {
        ...
        console.error("Failed to generate ebook intro", e);
        return "Welcome to this collection of our favorite recipes! \n\nInside, you'll find a handpicked selection of delicious meals designed to inspire your culinary journey. Whether you are a beginner or a seasoned cook, we hope these recipes bring joy and great flavor to your table.\n\nHappy cooking!";
    }
}

export async function generateEbookRecipeImage(apiKey: string, name: string): Promise<string> {
    const ai = new GoogleGenAI({ apiKey });
    
    const imagePrompt = `A beautiful, appetizing, and professional food photography shot of "${name}". High resolution, perfect lighting, isolated on a clean surface or rustic table. 16:9 aspect ratio, suitable for a recipe book.`;
    ...
    try {
        ...
        throw new Error("No image generated for recipe");
    } catch (e) {
