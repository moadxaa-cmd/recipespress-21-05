...
import { ImageGenerationOptions } from '../components/ImageGenerationOptions';
import { Icons } from '../constants';
import type { ImageConfiguration, ToastType, ToastMessage, View, SettingsTab, WordPressSite } from '../types';
...
export const CreatePost: React.FC<CreatePostProps> = ({ sites, isGenerating, selectedSiteId, setSelectedSiteId, handleGenerate, showToast }) => {
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    ...
                    ) : ( <button type="button" onClick={() => navigate('/settings')} className="text-sm text-teal-600 font-semibold hover:underline">Add a site to get started</button> )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6">
                    {!selectedSiteId ? (
                        <div className="p-3 bg-red-50 text-sm text-red-800 rounded-lg flex items-center gap-3 border border-red-200">
                            ...
                            <p>Please <button type="button" onClick={() => navigate('/settings')} className="font-bold underline hover:text-red-900 focus:outline-none">add and select a site</button> to enable generation.</p>
                        </div>
                    ) : <div />}
